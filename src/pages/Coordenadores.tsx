    ...
    const fetchProfessores = async () => {
      try {
        const response = await api.get<EmployeeSummary[]>(
          "employee/getAllByRole/COORDENADOR"
        );
  ...
  return (
    <AppLayout>
      <main className="col-start-2 row-start-2 p-4 md:p-8 overflow-auto bg-gray-50 pt-20 md:pt-4 h-full">
        <h2 className="subtitle font-semibold">Visão Geral de Coordenadores</h2>
        <p className="text-gray-600 mb-6">
          Lista de todos os coordenadores cadastrados e suas atividades.
        </p>
